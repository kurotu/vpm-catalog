...
> **注意**: vrc-getのrepositories.txt（`https://vrc-get.anatawa12.com/` 等）や、他のVPMキュレーションサイト（例: `https://remuria.natsuneko.com/`）は情報源にしてはならない。これらは他者が収集・整理したリストであり、ここから転用することはオリジナルの発見にならない。
...
```
https://api.github.com/users/xxx/repos?type=public&per_page=100&sort=updated
```

- ユーザーが存在しない場合（404）はこの経路をスキップして次のチェックへ進む。
- レスポンスの `Link` ヘッダに `rel="next"` が含まれる場合は、`page=2`, `page=3` … と順にフェッチして全リポジトリを取得する。
- `fork: false` な公開リポジトリは **`has_pages` で絞り込まず** VPM候補として Step 4 に送る。
  `has_pages: true` は GitHub Pages URL 推定の優先材料にはなるが必須条件ではない。`raw.githubusercontent.com/.../index.json` や `.../vpm.json` のような **Pagesを使わないJSON配布** もあるため、Step 4 のURL正規化・`web_fetch` 検証で判定する。
...
```
https://api.github.com/users/<user>/repos?type=public&per_page=100&sort=updated
```

- ユーザーが存在しない場合（404）はこの経路をスキップする。
- ページングが必要な場合は `page=2`, `page=3` … と順にフェッチする。
- `fork: false` な公開リポジトリは **`has_pages` で絞り込まず** VPM候補として Step 4 に送る。
...
BoothショップのサブドメインとGitHubユーザー名が完全一致しないケースがある
...
- **他のキュレーションサイトを情報源にしない**: `https://remuria.natsuneko.com/` などの第三者キュレーションサイトは情報源として使用しない。これらのサイトは独自のキュレーション活動の成果物であり、そこからURLを転用することはオリジナルの発見にならない
